...
export default function OrganizationCategoryProvidersPage() {
  const params = useParams();
  const categorySlug = params?.category as string;
  const { org, loading } = useOrganization();

  const { category, providers } = useMemo(() => {
    if (!org) return { category: null, providers: [] };
    ...
    // Filter providers
    const catProviders: any[] = [];
    if (org.members) {
      org.members.forEach((m: any) => {
        if (m.provider) {
          const pSkills = m.provider.providerSkills || [];
          const hasSkill = pSkills.some((ps: any) => ps.skill?.category?.slug === categorySlug);
          if (hasSkill) {
            catProviders.push({
              ...m.provider,
    ...
    return { category: catObj, providers: catProviders };
  }, [org, categorySlug]);
  ...
  const breadcrumb = [
    ...
    { label: category.name, href: `/organizations/${org.id}/catalog/${categorySlug}` },
    { label: "Ustalar" },
  ];
      ...
      <div className="mt-8 mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2">
            {org.name} tashkilotining {category.name} yo'nalishidagi barcha ustalari
          </p>
        </div>
      </div>
