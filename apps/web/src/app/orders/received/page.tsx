...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (res.data.success) {
        setOrders(res.data.data.filter((o: any) => isProvider(o)))
      }
  ...
  const handleFail = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!failModal || !failReason) return
    setFailLoading(true)
    try {
      await api.patch(`/orders/${failModal.id}/fail`, { reason: failReason })
      toast.success("Buyurtma muvaffaqiyatsiz deb belgilandi")
  ...
  const handleFinish = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch(`/orders/${finishModal.id}/complete`, { isCashPaid })
      toast.success("Buyurtma yakunlandi")
      setFinishModal(null); setIsCashPaid(false)
      fetchOrders()
    } catch (err: any) {
      toast.error(err.response?.data?.error || "Xatolik")
  ...
  const handleOnTheWay = async (orderId: string) => {
    try {
      await api.patch(`/orders/${orderId}/on-the-way`)
      toast.success("Mijozga yo'lga chiqqaningiz xabar qilindi")
      fetchOrders()
    } catch (err: any) {
      toast.error(err.response?.data?.error || "Xatolik")
    }
  ...
  return (
    <div className="flex flex-col gap-6 fade-in">
      <div className="flex items-center gap-4">
        <Link href="/orders" className="p-2 -ml-2 rounded-xl hover:bg-gray-100 text-gray-500 transition-colors">
          <ChevronRight className="w-5 h-5 rotate-180" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text)" }}>Kelgan buyurtmalar</h1>
          <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>
            Sizga kelgan jami {orders.length} ta buyurtma
          </p>
        </div>
      </div>
            ...
            return (
              <div key={order.id} className="glass-card p-5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2 mt-auto">
                  ...
                  {provider && (order.status === "ACCEPTED" || order.status === "IN_PROGRESS") && (
                    <div className="flex flex-col gap-1.5 mt-1">
                      {order.status === "ACCEPTED" && (
                        <button onClick={() => handleOnTheWay(order.id)}
                          className="py-2.5 rounded-xl text-sm font-bold bg-indigo-500/10 text-indigo-600 hover:bg-indigo-500/20 transition-colors flex items-center justify-center gap-2">
                          <Zap size={15} /> Yo'lga chiqdim
                        </button>
                      )}
                      <div className="flex gap-1.5">
                        <button onClick={() => { setFinishModal(order); setIsCashPaid(false) }}
                          className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors flex items-center justify-center gap-2">
                          <CheckCircle size={15} /> Yakunlash
                        </button>
                        <button onClick={() => { setFailModal(order); setFailReason("") }}
                          className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-red-500/10 text-red-600 hover:bg-red-500/20 transition-colors flex items-center justify-center gap-2">
                          <XCircle size={15} /> Bajara olmadim
                        </button>
                      </div>
                    </div>
                  )}
          ...
          <form onSubmit={handleCancel} className="glass-modal fade-in p-6 w-full max-w-md flex flex-col gap-4">
            ...
            <div>
              ...
              <select required value={cancelReason} onChange={e => setCancelReason(e.target.value)}
                className="glass-input w-full p-2.5 bg-transparent border border-[var(--border-strong)] rounded-xl outline-none text-sm">
                <option value="">-- Sababni tanlang --</option>
                <option value="Fikrimdan qaytdim">Fikrimdan qaytdim</option>
                <option value="Boshqa usta topdim">Boshqa usta topdim</option>
                <option value="Provayder kelmadi">Provayder kelmadi</option>
      ...
      {failModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleFail} className="glass-modal fade-in p-6 w-full max-w-md flex flex-col gap-4">
            <h3 className="text-lg font-bold flex items-center gap-2" style={{ color: "var(--text)" }}>
              <XCircle size={20} className="text-red-500" /> Xizmatni bekor qilish (Muvaffaqiyatsiz)
            </h3>
            <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
              Sababni ko'rsatganingizdan so'ng buyurtma yopiladi va hamyoningiz/reytingingiz qayta hisoblanadi.
            </p>
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: "var(--text-secondary)" }}>
                Muvaffaqiyatsizlik sababi *
              </label>
              <select required value={failReason} onChange={e => setFailReason(e.target.value)}
                className="glass-input w-full p-2.5 bg-transparent border border-[var(--border-strong)] rounded-xl outline-none text-sm">
                <option value="">-- Sababni tanlang --</option>
                <option value="Men bajara olmadim">Men bajara olmadim (Mening aybim)</option>
                <option value="Mijoz yo'q / kelmadi">Mijoz yo'q / kelmadi</option>
                <option value="Material / ehtiyot qism yo'q">Material / ehtiyot qism yo'q</option>
                <option value="Boshqa sabab">Boshqa sabab</option>
              </select>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => setFailModal(null)}
                className="flex-1 py-2.5 rounded-xl text-sm font-medium btn-ghost">Bekor qilish</button>
              <button type="submit" disabled={failLoading || !failReason}
                className="flex-1 py-2.5 rounded-xl text-sm font-bold btn-danger disabled:opacity-60">
                {failLoading ? <Loader2 size={16} className="animate-spin mx-auto" /> : "Tasdiqlash"}
              </button>
            </div>
          </form>
        </div>
      ...
      {/* Provider Finish Modal */}
      {finishModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleFinish} className="glass-modal fade-in p-6 w-full max-w-md flex flex-col gap-4">
            <h3 className="text-lg font-bold text-emerald-500 flex items-center gap-2">
              <CheckCircle size={20} /> Xizmatni yakunlash
            </h3>
            <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
              Xizmatni yakunlaganingizdan so'ng, tizim hamyoningizdan komissiya yechib oladi. Mijoz naqd to'lov qildimi?
            </p>
            <div className="flex items-center gap-3 p-4 rounded-xl border border-[var(--border-strong)] bg-[var(--bg-secondary)]">
              <input type="checkbox" id="cash-paid" checked={isCashPaid}
                onChange={e => setIsCashPaid(e.target.checked)}
                className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
              <label htmlFor="cash-paid" className="font-medium cursor-pointer flex-1" style={{ color: "var(--text)" }}>
                Ha, mijoz xizmat uchun naqd to'lov qildi
              </label>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => setFinishModal(null)}
                className="flex-1 py-2.5 rounded-xl text-sm font-medium btn-ghost">Ortga</button>
              <button type="submit" disabled={finishLoading}
                className="flex-1 py-2.5 rounded-xl text-sm font-bold btn-success disabled:opacity-60">
                {finishLoading ? <Loader2 size={16} className="animate-spin mx-auto" /> : "Yakunlash"}
              </button>
            </div>
          </form>
        </div>
