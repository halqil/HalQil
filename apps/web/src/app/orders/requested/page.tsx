...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (res.data.success) {
        setOrders(res.data.data.filter((o: any) => o.user?.id === user?.id || !isProvider(o)))
      }
  ...
  return (
    <div className="flex flex-col gap-6 fade-in">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--text)" }}>Yuborilgan buyurtmalar</h1>
          <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>
            Siz yuborgan jami {orders.length} ta buyurtma
          </p>
        </div>
      </div>
