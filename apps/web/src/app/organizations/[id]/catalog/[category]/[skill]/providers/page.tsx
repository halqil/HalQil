...
export default function OrganizationSkillProvidersPage() {
  const params = useParams();
  const categorySlug = params?.category as string;
  const skillSlug = params?.skill as string;
  const { org, loading } = useOrganization();

  const { category, skill, providers } = useMemo(() => {
    if (!org) return { category: null, skill: null, providers: [] };

    // Find category and skill name for breadcrumb
    let foundCat = null;
    let foundSkill = null;
    if (org.skills) {
    ...
    return { category: foundCat, skill: foundSkill, providers: skillProviders };
      ...
      <div className="mt-8 mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2">
            {org.name} tashkilotining {skill.name} yo'nalishidagi barcha ustalari
          </p>
        </div>
      </div>
