...
export default function CategoryProvidersPage() {
  ...
  useEffect(() => {
    ...
    // Fetch providers
    api.get(`/catalog/categories/${category}/providers`)
      .then(res => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 fade-in">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">{categoryName} ustalari</h1>
        <p className="text-gray-500 mt-2">Bu yo'nalishdagi barcha tasdiqlangan mutaxassislar ro'yxati</p>
      </div>
      ...
      {loading ? (
        ...
        <ProviderList 
          providers={providers} 
          emptyMessage="Bu kategoriyada hozircha ustalar ro'yxatdan o'tmagan." 
        />
      )}
    </div>
  )
